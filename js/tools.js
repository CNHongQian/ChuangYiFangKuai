...
let toolsCurrentFormatFilter = 'all'; // 添加格式筛选变量
let tagsData = []; // 存储标签数据
        ...
        if (data && data.length > 0) {
            ...
            toolsPageData = data
                .filter(item => {
                    ...
                    // 检查category值的确切类型
                    const isTool = item.category === 'tool' || item.type === 'tool';
                    console.log('是否为工具:', isTool, 'category值:', typeof item.category, 'category值:', item.category);
                    return isTool;
                })
                .map(item => ({
                    ...item,
                    type: item.type || item.category || 'tool', // 使用category作为type的备用值
                    downloads: item.downloads || 0,
                    likes: item.likes || 0,
                    views: item.views || 0
                }));
            
            console.log('从GitHub加载的工具数据:', toolsPageData);
            
            if (toolsPageData.length > 0) {
                // 动态生成格式选项
                generateFormatOptions();
                
                // 确保数据加载完成后渲染
                console.log('GitHub数据加载完成，开始渲染，数据量:', toolsPageData.length);
                renderTools();
                return;
            } else {
                console.log('GitHub数据中没有找到工具类型的项目');
            }
        } else {
...
function createCustomDropdown(selectId, formats) {
    ...
    menu.addEventListener('click', (e) => {
        ...
        if (originalSelect) {
            ...
            // 直接调用格式筛选处理函数
            toolsCurrentFormatFilter = selectedValue;
...
async function loadSampleData() {
    try {
        ...
        const data = await response.json();
        // 过滤出工具类型的数据
        toolsPageData = data.filter(item => item.type === 'tool');
    } catch (error) {
...
function setupEventListeners() {
    ...
    nextBtn.addEventListener('click', () => {
        const totalPages = Math.ceil(filteredData.length / toolsItemsPerPage);
...
function updateWorkCount() {
    ...
    if (workCountElement) {
        const totalCount = toolsPageData.length;
...
function renderTools() {
    ...
    // 应用格式筛选
    if (toolsCurrentFormatFilter !== 'all') {
        filteredData = filteredData.filter(item => 
            item.fileFormat && item.fileFormat.toLowerCase() === toolsCurrentFormatFilter.toLowerCase()
        );
    }
        ...
        if (searchTerm) {
            ...
            if (formatSelect) {
                ...
                if (matchedFormat) {
                    ...
                    toolsCurrentFormatFilter = matchedFormat.value;
                }
            }
        }
    }
...
function renderPaginatedItems() {
    ...
    // 计算当前页的起始和结束索引
    const startIndex = (currentPage - 1) * toolsItemsPerPage;
    const endIndex = startIndex + toolsItemsPerPage;
...
function renderPagination() {
    ...
    // 更新分页信息
    const startItem = filteredData.length === 0 ? 0 : (currentPage - 1) * toolsItemsPerPage + 1;
    const endItem = Math.min(currentPage * toolsItemsPerPage, filteredData.length);
...
function createToolCard(tool) {
    ...
    if (tool.image && tool.image.trim() !== '') {
        // 如果是相对路径，添加CDN前缀
        if (!tool.image.startsWith('http')) {
            imagePath = 'https://cdn.jsdelivr.net/gh/CNHongQian/ChuangYiFangKuai@main/' + tool.image;
        } else {
    ...
    cardInner.innerHTML = `
        <img src="${imagePath}" alt="${tool.title}" class="building-image" onerror="this.src='https://cdn.jsdelivr.net/gh/CNHongQian/ChuangYiFangKuai@main/img/none.png'">
...
function handleFormatFilter(event) {
    toolsCurrentFormatFilter = event.target.value;
    console.log('格式筛选触发，选择的格式:', toolsCurrentFormatFilter);
...
// 跳转到详情页面
function goToDetail(tool) {
    // 保存当前作品到localStorage
    localStorage.setItem('currentWork', JSON.stringify(tool));
    
    // 跳转到详情页
    window.location.href = `detail.html?id=${tool.id}`;
}

// 从卡片分享作品
function shareWorkFromCard(tool) {
    ...
    if (isInPagesDir) {
        ...
        detailUrl = `detail.html?id=${tool.id}`;
    } else {
        // 如果在根目录，使用完整路径
        detailUrl = `pages/detail.html?id=${tool.id}`;
    }
