...
let commandCurrentFormatFilter = 'all'; // 添加格式筛选变量
let tagsData = []; // 存储标签数据
    ...
    try {
        ...
        if (data && data.length > 0) {
            ...
            commandPageData = data
                ...
                .map(item => ({
                    ...item,
                    type: item.type || item.category || 'command', // 使用category作为type的备用值
...
function setupEventListeners() {
    ...
    nextBtn.addEventListener('click', () => {
        const totalPages = Math.ceil(filteredData.length / commandItemsPerPage);
...
function updateWorkCount() {
    ...
    if (workCountElement) {
        const totalCount = commandPageData.length;
...
function renderCommands() {
    const grid = document.getElementById('commandsGrid');
    if (!grid) {
        console.error('找不到 commandsGrid 元素');
        return;
    }
    ...
    // 应用格式筛选
    if (commandCurrentFormatFilter !== 'all') {
        filteredData = filteredData.filter(item => 
            item.fileFormat && item.fileFormat.toLowerCase() === commandCurrentFormatFilter.toLowerCase()
        );
    }
        ...
        if (searchTerm) {
            ...
            if (formatSelect) {
                ...
                if (matchedFormat) {
                    ...
                    commandCurrentFormatFilter = matchedFormat.value;
                }
            }
        }
    }
...
function renderPaginatedItems() {
    ...
    // 计算当前页的起始和结束索引
    const startIndex = (currentPage - 1) * commandItemsPerPage;
    const endIndex = startIndex + commandItemsPerPage;
...
function renderPagination() {
    ...
    // 更新分页信息
    const startItem = filteredData.length === 0 ? 0 : (currentPage - 1) * commandItemsPerPage + 1;
    const endItem = Math.min(currentPage * commandItemsPerPage, filteredData.length);
...
function createCommandCard(command) {
    ...
    if (command.image && command.image.trim() !== '') {
        // 如果是相对路径，添加CDN前缀
        if (!command.image.startsWith('http')) {
            imagePath = 'https://cdn.jsdelivr.net/gh/CNHongQian/ChuangYiFangKuai@main/' + command.image;
        } else {
