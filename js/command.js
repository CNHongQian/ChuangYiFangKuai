...
function renderCommand() {
    ...
    if (itemsToDisplay.length === 0) {
        grid.innerHTML = '<div style="text-align: center; padding: 3rem; color: #666;">没有找到相关指令作品</div>';
    }
...
function createCommandCard(command) {
    ...
    const typeTag = command.type === 'tool' ? '工具' : '指令';
    const categoryTag = getCategoryName(command.category);
    
    cardInner.innerHTML = `
        <img src="${command.image}" alt="${command.title}" class="building-image">
        <div class="building-info">
            <h3 class="building-title">${command.title}</h3>
            <p class="building-author">作者: ${command.author}</p>
            <div class="building-stats">
                <span class="building-tag">${typeTag}</span>
                <span class="building-tag">${categoryTag}</span>
                <span><i class="fas fa-file"></i> ${command.fileSize}</span>
...
function shareWorkFromCard(command) {
    ...
    const workType = command.type === 'command' ? '指令' : '建筑';
